# Plots

## 02_12_spatial_dist.png

![02_12_spatial_dist.png](plots/02_12_spatial_dist.png)

## hot_regions_200_s.png

![hot_regions_200_s.png](plots/hot_regions_200_s.png)

## template_creation.png

![template_creation.png](plots/template_creation.png)

## hot_regions_5s.png

![hot_regions_5s.png](plots/hot_regions_5s.png)

## double_burst_e2_3943.png

![double_burst_e2_3943.png](plots/double_burst_e2_3943.png)

## events_per_time_frame_hist.png

![events_per_time_frame_hist.png](plots/events_per_time_frame_hist.png)

## bayesfactorlimits_3_zoomed.png

![bayesfactorlimits_3_zoomed.png](plots/bayesfactorlimits_3_zoomed.png)

## transients_scatter_rotated.png

![transients_scatter_rotated.png](plots/transients_scatter_rotated.png)

## crossmatch_seperations.png

![crossmatch_seperations.png](plots/crossmatch_seperations.png)

## bayes_successrate_timebinning2.png

![bayes_successrate_timebinning2.png](plots/bayes_successrate_timebinning2.png)

## bayesfactorlimits_3.png

![bayesfactorlimits_3.png](plots/bayesfactorlimits_3.png)

## number_of_isolated_flares.png

![number_of_isolated_flares.png](plots/number_of_isolated_flares.png)

## transients_hist_rotated.png

![transients_hist_rotated.png](plots/transients_hist_rotated.png)

## bayes_eclipse_successrate_depth.png

![bayes_eclipse_successrate_depth.png](plots/bayes_eclipse_successrate_depth.png)

## HD_3447_large.png

![HD_3447_large.png](plots/HD_3447_large.png)

## check_eclipse_estimate_success2.png

![check_eclipse_estimate_success2.png](plots/check_eclipse_estimate_success2.png)

## B_eclipse.png

![B_eclipse.png](plots/B_eclipse.png)

## cluster_radius_n_sources.png

![cluster_radius_n_sources.png](plots/cluster_radius_n_sources.png)

## B_values_3d.png

![B_values_3d.png](plots/B_values_3d.png)

## check_estimate_success1.png

![check_estimate_success1.png](plots/check_estimate_success1.png)

## Significance_plot.png

![Significance_plot.png](plots/Significance_plot.png)

## source_against_n_counts_filter.png

![source_against_n_counts_filter.png](plots/source_against_n_counts_filter.png)

## check_eclipse_estimate_success1.png

![check_eclipse_estimate_success1.png](plots/check_eclipse_estimate_success1.png)

## Flux_comparison.png

![Flux_comparison.png](plots/Flux_comparison.png)

## bayes_rate_estimate.png

![bayes_rate_estimate.png](plots/bayes_rate_estimate.png)

## bayesfactorlimits_5_zoomed.png

![bayesfactorlimits_5_zoomed.png](plots/bayesfactorlimits_5_zoomed.png)

## crossmatch_pie_chart.png

![crossmatch_pie_chart.png](plots/crossmatch_pie_chart.png)

## gaia_hr_diagram.png

![gaia_hr_diagram.png](plots/gaia_hr_diagram.png)

## B_peak_threshold_vs_N_reg.png

![B_peak_threshold_vs_N_reg.png](plots/B_peak_threshold_vs_N_reg.png)

## total_lc_counts_hist.png

![total_lc_counts_hist.png](plots/total_lc_counts_hist.png)

## bayesfactorlimits_5.png

![bayesfactorlimits_5.png](plots/bayesfactorlimits_5.png)

## dr14_offsets.png

![dr14_offsets.png](plots/dr14_offsets.png)

## HD_3447.png

![HD_3447.png](plots/HD_3447.png)

## mean_count_histogram.png

![mean_count_histogram.png](plots/mean_count_histogram.png)

## transients.png

![transients.png](plots/transients.png)

## plot_some_n_bayes.png

![plot_some_n_bayes.png](plots/plot_some_n_bayes.png)

## crossmatch_sep.png

![crossmatch_sep.png](plots/crossmatch_sep.png)

## B_factor_vs_sigma.png

![B_factor_vs_sigma.png](plots/B_factor_vs_sigma.png)

## HD_3447_zoom.png

![HD_3447_zoom.png](plots/HD_3447_zoom.png)

## B_eclipse_histogram.png

![B_eclipse_histogram.png](plots/B_eclipse_histogram.png)

## unique_reg.png

![unique_reg.png](plots/unique_reg.png)

## check_estimate_success2.png

![check_estimate_success2.png](plots/check_estimate_success2.png)

## B_values_distribution_all_regions.png

![B_values_distribution_all_regions.png](plots/B_values_distribution_all_regions.png)

## unique_regions_aitoff.png

![unique_regions_aitoff.png](plots/unique_regions_aitoff.png)

## HD_200560_lc_10s.png

![HD_200560_lc_10s.png](plots/HD_200560_lc_10s.png)

## accepted_n_values.png

![accepted_n_values.png](plots/accepted_n_values.png)

## crossmatch_confusion_matrix.png

![crossmatch_confusion_matrix.png](plots/crossmatch_confusion_matrix.png)

## unique_sources.png

![unique_sources.png](plots/unique_sources.png)

## mean_observed_count_rate.png

![mean_observed_count_rate.png](plots/mean_observed_count_rate.png)

## data_cube_frame_with_inset_and_box.png

![data_cube_frame_with_inset_and_box.png](plots/data_cube_frame_with_inset_and_box.png)

## data_cube_frame.png

![data_cube_frame.png](plots/data_cube_frame.png)

## total_lc_counts_hist_small_n.png

![total_lc_counts_hist_small_n.png](plots/total_lc_counts_hist_small_n.png)

## bayes_successrate_timebinning1.png

![bayes_successrate_timebinning1.png](plots/bayes_successrate_timebinning1.png)

## SIMBAD_seperations.png

![SIMBAD_seperations.png](plots/SIMBAD_seperations.png)

## bayesfactorlimits_3_5.png

![bayesfactorlimits_3_5.png](plots/bayesfactorlimits_3_5.png)

## B_peak.png

![B_peak.png](plots/B_peak.png)

## B_peak_histogram.png

![B_peak_histogram.png](plots/B_peak_histogram.png)

## N_mu_dist_all_lc.png

![N_mu_dist_all_lc.png](plots/N_mu_dist_all_lc.png)

## OM_magnitudes.png

![OM_magnitudes.png](plots/OM_magnitudes.png)

## SIMBAD_counterparts_classification.png

![SIMBAD_counterparts_classification.png](plots/SIMBAD_counterparts_classification.png)

## simbad_exod_subclass_piechart.png

![simbad_exod_subclass_piechart.png](plots/simbad_exod_subclass_piechart.png)

## hot_regions_50_s.png

![hot_regions_50_s.png](plots/hot_regions_50_s.png)

## hot_regions_5_s.png

![hot_regions_5_s.png](plots/hot_regions_5_s.png)

