...
def compute_background_two_templates(cube, rejected):
    ...
    lightcurve_no_source_image = np.array([np.sum(np.where(image_total<threshold, cube[:,:,i], 0),
                                         axis=(0,1)) for i in range(cube.shape[2])])
    background = np.empty(cube.shape)
    background[:,:,kept] = normalized_GTI_background[:,:,np.newaxis]*lightcurve_no_source_image[kept]
    background[:,:,rejected] = normalized_BTI_background[:,:,np.newaxis]*lightcurve_no_source_image[rejected]

    background_withsource = np.empty(cube.shape)
    background_withsource[:,:,kept] = normalized_GTI_background[:,:,np.newaxis]*lightcurve_no_source_image[kept]\
                                            + source_only_image_GTI[:,:,np.newaxis]/(len(kept))
    background_withsource[:,:,rejected] = normalized_BTI_background[:,:,np.newaxis]*lightcurve_no_source_image[rejected]\
                                            + source_only_image_BTI[:,:,np.newaxis]/(len(rejected))
    return background, background_withsource
...
def experimental_variability_Poisson_likelihood(cube,background_withsource):
    # likelihood_cube = -poisson.logpmf(cube, background_withsource)
    variabibility_map = np.nanmax(-poisson.logpmf(cube, background_withsource), axis=2)
    return variabibility_map
...
if __name__=="__main__":
    ...
    #background_images_new, background_withsource_new=compute_background_two_templates(cube, rejected)
    variability_map=experimental_variability_Poisson_likelihood(cube)
    print(size_arcsec, time_interval)
    plt.figure()
    plt.imshow(variability_map, interpolation='none', norm=LogNorm())
    plt.savefig(os.path.join(data_processed, '0831790701', f"TestNewVar.png"))
