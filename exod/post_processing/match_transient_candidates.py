...
def Gaia_lookup(tab_ra, tab_dec):
    ...
    for (ra, dec) in zip(tab_ra, tab_dec):
        ...
        if len(result_table)>0:
            result = result_table[0]
            tab_Gaia_name.append(list(result["Source"]))
            tab_dist.append(list(result["Dist"].value))
            tab_sep.append(SkyCoord(ra=ra, dec=dec, unit=(u.deg, u.deg), frame='icrs').separation(
                SkyCoord(ra=result["RA_ICRS"], dec=result["DE_ICRS"], unit=(u.deg, u.deg), frame='icrs')
                                              ).to(u.arcsec).value)
        else:
