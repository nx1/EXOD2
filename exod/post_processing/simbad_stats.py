...
from exod.utils.path import data_util, data_catalogue 
...
def rename_cols(df):
    ...
    df = df.sort_values('object_count', ascending=False)
    return df
...
savedir = Path('/home/nkhan/EXOD2/data/results_combined/simbad_stats')
...
dfs = {'SIMBAD'                   : df_otypes,
       'DR14 FULL'                : df_dr14,
       'DR14 TRANSIENT'           : df_dr14_transient,
...
for k, df in dfs.items():
    ...
    print(f'\nTop 20 otypes by object count:')
    print(f'===============================')
    print(df[['otype', 'simbad_subclass', 'description', 'object_count',  'percentage']].head(20))
...
print('==============================')
print(df_perc.sort_values('SIMBAD', ascending=False).head(20))
...
print('=================================')
print(df_count.sort_values('SIMBAD', ascending=False).head(20))
df_count.to_csv(savedir / 'otype_stats_count.csv', index=False)
print(f'Saved to {savedir / "otype_stats_count.csv"}')



# [print(c) for c in df_count['otype'].values if c not in df_otypedef['label'].values]
