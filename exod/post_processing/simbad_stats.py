...
def load_dfs():
    # Statistics for the whole SIMBAD database
    ...
    tab_exod_cat_dr14_transient      = tab_exod_cat[tab_exod_cat['DR14_SC_VAR_FLAG'] == 1]
    tab_exod_cat_dr14_not_transient  = tab_exod_cat[(tab_exod_cat['DR14_SC_VAR_FLAG'] == 0) & (tab_exod_cat['DR14_SRCID'] != 0)]
    n_exod_cat = len(tab_exod_cat)

    # DR14 Statistics
    #tab_dr14           = Table.read(data_util      / '4xmmdr14slim_cmatch_simbad.fits')
    tab_dr14                      = Table.read(data_util      / '4xmmdr14slim_cmatch_simbad_r_10.fits')
    n_dr14                        = len(tab_dr14)
    n_dr14_transient              = len(tab_dr14[tab_dr14['SC_VAR_FLAG'] == 1])
    n_exod_cat_dr14_transient     = len(tab_exod_cat_dr14_transient)
    n_exod_cat_dr14_not_transient = len(tab_exod_cat_dr14_not_transient)

    tab_dr14           = tab_dr14[tab_dr14['otype_x'] != '    '] # Remove null rows.
    tab_dr14_transient = tab_dr14[tab_dr14['SC_VAR_FLAG'] == 1]
    ...
    df_dr14_transient              = process_table(tab_dr14_transient,              sumtotal=n_dr14_transient)
    df_exod_cat                    = process_table(tab_exod_cat,                    sumtotal=n_exod_cat)
    df_exod_cat_dr14_transient     = process_table(tab_exod_cat_dr14_transient,     sumtotal=n_exod_cat_dr14_transient)
    df_exod_cat_dr14_not_transient = process_table(tab_exod_cat_dr14_not_transient, sumtotal=n_exod_cat_dr14_not_transient)
    
    
    dfs = {'SIMBAD'                     : df_otypes,
           'DR14'                       : df_dr14,
           'DR14 & SC_VAR_FLAG=1'       : df_dr14_transient,
           'EXOD FULL'                  : df_exod_cat,
           'EXOD & SC_VAR_FLAG=1'       : df_exod_cat_dr14_transient,
           'EXOD & SC_VAR_FLAG=0'       : df_exod_cat_dr14_not_transient}
    return dfs
...
def main():
    ...
    print('==============================')
    print(df_perc.sort_values('EXOD FULL', ascending=False))
    ...
    print('=================================')
    print(df_count.sort_values('EXOD FULL', ascending=False))
    df_count.to_csv(savedir / 'otype_stats_count.csv', index=False)
    print(f'Saved to {savedir / "otype_stats_count.csv"}')


    
    # Create a new dataframe by combining both count and percentage data
    df_perc  = df_perc.fillna(0).sort_values('EXOD FULL', ascending=False)
    df_count = df_count.fillna(0).sort_values('EXOD FULL', ascending=False)
    df_combined = df_count.copy()
    print(df_combined)

    # Iterate through each column in the count dataframe
    for column in df_count.columns:
        if column == 'otype':
            continue
        # Format each cell with the integer count followed by the percentage in brackets and with a % sign
        df_combined[column] = df_count[column].astype(int).apply(lambda x: f"{x:,}") + ' (' + df_perc[column].round(2).astype(str) + '%)'
    
    
    # Display the combined dataframe
    df_combined['otype_long'] = df_combined['otype'].apply(lambda x: df_otypedef[df_otypedef['label'] == x]['description'].values[0] if x in df_otypedef['label'].values else x)
    print('\n\n')
